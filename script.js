...
/* =======================================
   🌟 PROCEED BUTTON → SHOW CALCULATOR
========================================*/
...
// Hide app until proceed
document.querySelector(".app").style.display = "none";



// Full dataset (COMPLETE, EXACTLY as you provided)
const data = {
...
"2-Octanone": {"Tc": 632.7, "Pc": 2.64, "omega": 0.454874},
"3-Octanone": {"Tc": 627.7, "Pc": 2.704, "omega": 0.440561},
"1-Octene": {"Tc": 566.9, "Pc": 2.663, "omega": 0.392149},
"Octyl Mercaptan": {"Tc": 667.3, "Pc": 2.52, "omega": 0.449744},
"1-Octyne": {"Tc": 574, "Pc": 2.88, "omega": 0.42329},
...
const loadingInterval = setInterval(() => {
  loadProgress += 2; // 2% every 100ms → full 5 seconds total
  ...
  if (loadProgress >= 100) {
    ...
    // show intro only when fully loaded
...
// Hide calculator until intro is done
document.querySelector(".app").style.display = "none";

if (typeof data === "undefined") {
  console.error("ERROR: `data` object not found.");
}

/** DOM elements **/
const numSpeciesSelector = document.getElementById("numSpecies");
...
// Use the keys of data sorted alphabetically for dropdown options
const gasNames = Object.keys(data).sort((a,b)=>a.localeCompare(b));

/** Initial render */
generateSpeciesBoxes(1);
numSpeciesSelector.value = "1";

/** Listeners **/
numSpeciesSelector.addEventListener("change", () => {
  const count = parseInt(numSpeciesSelector.value, 10) || 1;
  generateSpeciesBoxes(count);
});
...
calculateBtn.addEventListener("click", () => {
  ...
  for (let i=1; i<=count; i++){
    const select = document.getElementById(`gas${i}`);
    const yInput = document.getElementById(`y${i}`);
    if (!select || !yInput) continue;
    ...
    speciesList.push({ index: i, name, y, props });
  }

  let normalized = false;
  if (sumY === 0) {
    const eq = 1.0 / speciesList.length;
    speciesList.forEach(s => s.y = eq);
    normalized = true;
  } else if (Math.abs(sumY - 1.0) > 1e-6) {
    speciesList.forEach(s => s.y = s.y / sumY);
    normalized = true;
  }

  const T = parseFloat(Tinput.value) || 300;
  const P = parseFloat(Pinput.value) || 1;

  // PITZER CALCULATIONS
  speciesList.forEach(sp => {
    const { Tc, Pc, omega } = sp.props;

    sp.Tr = T / Tc;
    sp.Pr = P / Pc;

    sp.B0 = 0.083 - (0.422 / Math.pow(sp.Tr, 1.6));
    sp.B1 = 0.139 - (0.172 / Math.pow(sp.Tr, 4.2));

    const B = sp.B0 + omega * sp.B1;
    const lnphi = (B * sp.Pr) / sp.Tr;

    sp.phi = Math.exp(lnphi);
    sp.fugacity = sp.y * sp.phi * P;
  });

  renderResults(speciesList, {T, P, normalized, sumY});
});
calculateBtn.addEventListener("click", () => {
  const count = parseInt(numSpeciesSelector.value, 10) || 1;
...
/* ==========================================================
   VALIDATION FUNCTION — AUTO LIMIT + WARNING
   ========================================================== */
function attachValidation(i) {
  const yInput = document.getElementById(`y${i}`);
  ...
  yInput.addEventListener("input", () => {
    ...
    if (value > 1) {
      warn.textContent = "The value must be less than or equal to 1.";
      yInput.style.border = "2px solid red";
    } else {
      warn.textContent = "";
      yInput.style.border = "";
    }

    // 🔥 Check warnings every time
    checkWarnings();
  });
}
...
function generateSpeciesBoxes(count){
  ...
  for (let i = 1; i <= count; i++) {
      ...
      box.innerHTML = `
          ...
          <div class="species-row">
              <select id="gas${i}">
                  ${gasNames.map(g => `<option value="${g}">${g}</option>`).join("")}
              </select>
              <input id="y${i}" type="number" step="0.0001" placeholder="Mole fraction (yi)">
          </div>
      `;
      ...
      // ⭐ ADD THIS — VALIDATION
      document.getElementById(`y${i}`).addEventListener("input", validateMoleFractions);
  }
  ...
  validateMoleFractions();
    ...
    const sel = box.querySelector(`#gas${i}`);
    sel.innerHTML = gasNames.map(g =>
      `<option value="${escapeHtml(g)}">${escapeHtml(g)}</option>`
    ).join("");

    attachValidation(i);    // <<< ADDED — enables validation for each species

function checkWarnings() {
  let count = parseInt(numSpeciesSelector.value, 10) || 1;
...
function renderResults(speciesList, meta) {
  resultsDiv.innerHTML = "";

  const heading = document.createElement("h2");
  heading.textContent = "Multi-species calculation completed!";
  ...
  speciesList.forEach(s => {
    tbody.innerHTML += `
      <tr>
        ...
        <td>${s.y.toFixed(3)}</td>
        <td>${s.Tr.toFixed(3)}</td>
        <td>${s.Pr.toFixed(3)}</td>
        <td>${s.B0.toFixed(5)}</td>
        <td>${s.B1.toFixed(5)}</td>
        <td>${s.phi.toFixed(5)}</td>
        <td style="background:#dce9ff">${s.fugacity.toFixed(6)}</td>
      </tr>
    `;
  });
  ...
  cond.className = "note";
  cond.innerHTML = `<strong>Operating conditions:</strong> T = ${meta.T} K, P = ${meta.P} bar.`;
  resultsDiv.appendChild(cond);
}
...
function validateMoleFractions() {
    const count = parseInt(document.getElementById("numSpecies").value, 10);
    let sum = 0;

    for (let i = 1; i <= count; i++) {
        const yField = document.getElementById(`y${i}`);
        if (!yField) continue;
    ...
    // ⭐ DISABLE CALCULATE WHEN SUM > 1
   // ADD THIS INSIDE validateMoleFractions()

if (sum > 1) {
    warning.textContent = "⚠️ Mole fraction value must be less than or equal to 1.";
    calculateBtn.disabled = true;
    calculateBtn.style.cursor = "not-allowed";   // ⭐ ADD THIS
}
else {
    warning.textContent = "";
    calculateBtn.disabled = false;
    calculateBtn.style.cursor = "pointer";       // ⭐ RETURN TO NORMAL
}
}